...
3. Pro** 라이선스 모드를 선택하여 **의미 있는 이름(예: *mslearn-streaming*)으로 새 작업 영역을 만듭니다.
...
1. Azure Portal이 포함된 브라우저 탭으로 다시 전환하고 스크립트가 완료되면 dp203-*xxxxxxx *** 리소스 그룹이 프로비전된 지역을 **확인합니다.
...
1. **스트림 주문** 개요 페이지에서 입력** 페이지를 선택하고 **입력** 추가 메뉴를 사용하여 **다음 속성이 포함된 **이벤트 허브** 입력을 추가합니다.
    - **입력 별칭**: `orders`
...
3. 주문 추적** 대시보드**에서 **&#9999; &#65039; 편집** 메뉴를 선택한 다음, +타일** 추가를 선택합니다**. 그런 다음 타일 추가 창에서 **사용자 지정 스트리밍 데이터를** 선택하고 **다음**을 선택합니다**.**

4. 사용자 지정 스트리밍 데이터 타일 추가 창의 **데이터 세트** 아래에서 **실시간 데이터 데이터** 세트를 선택하고 **다음**을 선택합니다**.**
