...
3. **Pro** 라이선스 모드를 선택하여 의미 있는 이름(예: *mslearn-streaming*)이 있는 새 작업 영역을 만듭니다.
...
1. Azure Portal이 포함된 브라우저 탭으로 다시 전환하고 스크립트가 완료되면 **dp203-*xxxxxxx*** 리소스 그룹이 프로비저닝된 지역을 확인합니다.
...
1. **스트림 입력 추가** 개요 페이지에서 **입력** 페이지를 선택하고 **입력 추가** 메뉴를 사용하여 다름 속성이 있는 **이벤트 허브** 입력을 추가합니다.
    - **입력 별칭**: `orders`
...
1. **stream-orders** Stream Analytics 작업에 대한 **출력** 페이지를 봅니다. 그런 다음 **출력 추가** 메뉴를 사용하여 다음 속성이 있는 **Power BI** 출력을 추가합니다.
...
3. **주문 추적** 대시보드에서 **&#9999;&#65039; 편집** 메뉴를 선택한 다음, **+ 타일 추가**를 선택합니다. 그런 다음, **타일 추가** 창에서 **사용자 지정 스트리밍 데이터**를 선택하고 **다음**을 선택합니다.

4. **사용자 지정 스트리밍 데이터 추가 타일** 창의 **데이터 세트** 아래에서 **realtime-data** 데이터 세트를 선택하고 **다음**을 선택합니다.
