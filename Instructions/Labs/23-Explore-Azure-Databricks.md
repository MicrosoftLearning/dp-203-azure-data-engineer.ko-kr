...
Azure Synapse Analytics와 마찬가지로 Azure Databricks *작업 영역*은 Azure에서 Databricks 클러스터, 데이터 및 리소스를 관리하기 위한 중심점을 제공합니다.
...
1. 사이드바에서 **(+) 새** 작업을 사용하여 **Notebook**을 만듭니다.
1. 기본 전자 필기장 이름(**제목 없는 전자 필기장 *[날짜]***)을 **제품 탐색** 으로 변경하고 **연결** 드롭다운 목록에서 클러스터를 선택합니다(시작하는 데 1분 정도 걸릴 수 있음).
1. [**products.csv**](https://raw.githubusercontent.com/MicrosoftLearning/dp-203-azure-data-engineer/master/Allfiles/labs/23/adventureworks/products.csv) 파일을 로컬 컴퓨터에 다운로드하여 **products.csv**저장합니다. 그런 다음 **제품 탐색** Notebook의 **파일** 메뉴에서 **DBFS에 데이터 업로드를** 선택합니다.
1. **데이터 업로드** 대화 상자에서 파일이 업로드될 **DBFS 대상 디렉터리를** 확인합니다. 그런 다음 **파일** 영역을 선택하고 다운로드한 **products.csv** 파일을 컴퓨터에 업로드합니다. 파일이 업로드되면 **다음**을 선택합니다.
1. **Notebook에서 파일 액세스 창에서** 샘플 PySpark 코드를 선택하고 클립보드에 복사합니다. 이를 사용하여 파일의 데이터를 DataFrame으로 로드합니다. 그런 후 **완료**를 선택합니다.
...
1. 기존 코드 셀 아래에서 아이콘을 **+** 사용하여 새 코드 셀을 추가합니다. 그런 다음, 새 셀에 다음 코드를 입력합니다.

    ```python
    display(df1)
    ```

1. 새 셀의 오른쪽 위에 ** 있는&#9656; 셀 실행** 메뉴 옵션을 사용하여 실행합니다. 이 코드는 다음과 유사한 데이터 프레임의 내용을 표시합니다.
...
1. 결과 표 위에서 **시각화**를 선택하고 **+** 선택하여 시각화 편집기를 확인한 다음, 다음 옵션을 적용합니다.
    - **시각화 유형**: 막대
    - **X 열**: 범주
    - **Y 열**: *새 열을 추가하고* **ProductID**를 선택합니다. **Count** *집계**를 적용*합니다.

    시각화를 저장하고 다음과 같이 Notebook에 표시되는지 확인합니다.

    ![범주별 제품 수를 보여 주는 가로 막대형 차트](./images/databricks-chart.png)

## 테이블 만들기 및 쿼리

Python 또는 Scala와 같은 언어를 사용하여 파일의 데이터로 작업하는 데 많은 데이터 분석이 편안하지만 관계형 데이터베이스를 기반으로 하는 많은 데이터 분석 솔루션이 빌드됩니다. 데이터가 테이블에 저장되고 SQL을 사용하여 조작되는 입니다.

1. **제품 탐색** Notebook의 이전에 실행한 코드 셀의 차트 출력 아래에서 아이콘을 **+** 사용하여 새 셀을 추가합니다.
...
4. *Touring Bikes* 범주에서 제품의 이름과 가격을 반환하는 SQL 코드가 포함된 새 셀을 실행합니다.
5. 왼쪽 탭에서 **카탈로그** 작업을 선택하고 **제품** 테이블이 기본 데이터베이스 스키마( **기본값**으로 명명됨)에서 만들어졌는지 확인합니다. Spark 코드를 사용하여 데이터 분석가가 데이터를 탐색하고 분석 보고서를 생성하는 데 사용할 수 있는 관계형 테이블의 스키마 및 사용자 지정 데이터베이스 스키마를 만들 수 있습니다.
