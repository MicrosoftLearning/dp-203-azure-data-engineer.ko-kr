...
구독이 [정의된 테넌트에 대한 관리 수준 권한 및 단독 액세스 권한이 있는 Azure](https://azure.microsoft.com/free) 구독이 필요합니다.
...
레이크 데이터베이스는 Synapse SQL 서버리스 SQL 풀 및 Apache Spark에서 액세스할 수 있으므로 사용자는 스토리지를 컴퓨팅과 분리할 수 있습니다. 레이크 데이터베이스의 메타데이터를 사용하면 여러 엔진이 통합된 환경을 제공하고 데이터 레이크에서 지원되지 않는 추가 정보(예: 관계)를 쉽게 사용할 수 있습니다.

1. 레이크 데이터베이스를 만들려면 먼저 적절한 리소스 그룹에서 synapsexxxxxxx를 연 다음 Synapse Studio** 열기에서 **열린*** 링크를 클릭합니다***. 
...
4. 데이터 패널에서 Data라는 단어**의 오른쪽에 있는 + 기호를 클릭하고 Lake 데이터베이스를 선택합니다 ***.*****
   
    ![초기 lakedb 데이터베이스 만들기](./images/lakedb-configure.png)

> **참고**: 확인** 단추를 클릭하기 전에 읽고 이해해야 하는 Azure Synapse 데이터베이스 템플릿 사용** 약관에 대한 프롬프트**가 **표시됩니다.

5. 맨 오른쪽에는 속성 창 있습니다.
   1. 이름 필드에 lakedb**를 입력**합니다.
   1. 입력 폴더**에서 **폴더를 선택하고 루트/파일/데이터로 이동한 다음 확인을** 누릅니**다.

>**참고**: 입력 폴더**를 열 **때 오류가 표시될 수 있습니다. 그런 경우 확인을** 클릭하기 전에 루트 폴더를 두 번 클릭하고 **데이터로 작업하기만 하면 됩니다.

   1. 이 화면의 왼쪽에는 아래에 +테이블**이 있는 lakedb** 이름이 **있는 **기둥이 표시되고, 이를 클릭하고, 데이터 레이크에서 선택합니다 ***.***
   1. ***외부 테이블 이름*** 형식: **제품**.
   1. ***연결된 서비스에서*** 기본 옵션을 선택합니다.
   1. ***입력 파일 또는 폴더*** 내에서 맨 오른쪽의 파일 폴더를 클릭하고 데이터 > > 루트 >** 파일을 찾아 **products.csv***를 선택하고 ***확인을** 클릭한 **다음 계속**을 클릭합니다**.

6. 새 외부 테이블 창에서 **첫 번째 행 옵션을 ***선택하여 열 이름을*** 유추한 다음 만들기**를 클릭합니다**.**

![외부 원본 설정](./images/lakedb-external-table-source.png)

7. 쿼리 디자인 창의 **맨 위에 있는 게시** 를 누릅니다.
8. 데이터에서 왼쪽의 **작업 영역** 영역에 있는지 확인하고 Lake 데이터베이스** 섹션, **lakedb**를 확장**한 다음 ***Products** 테이블의 **오른쪽에*** 마우스를 놓고 상위 100개 행***을 선택합니다***.****

![외부 원본에서 파생된 테이블 구조](./images/lakedb-external-table-definition.png)

> **참고**: 커넥트** 기본 제공**으로 **나열되어 있는지 확인하고 **마스터** 데이터베이스를 선택한 상태로 두**거나 오른쪽의 새로 고침 단추를 클릭하고 lakedb** 데이터베이스를 선택할 **수 있습니다. 3부 명명 규칙 [데이터베이스]를 사용합니다. [스키마]. [table] 둘 다 작동합니다.

![레이크 데이터베이스의 첫 번째 외부 쿼리](./images/lakedb-first-external-query.png)

9. **실행** 단추를 눌러 Lake 데이터베이스 테이블 내의 데이터를 봅니다.

## 계정에 Microsoft Purview 서비스 추가

Microsoft Purview는 데이터 거버넌스, 정보 보호, 위험 관리 및 규정 준수 솔루션에 걸친 포괄적인 제품 포트폴리오입니다. 온-프레미스, 다중 클라우드 및 SaaS(Software as a Service) 데이터 전체에서 전체 데이터 자산을 제어, 보호 및 관리하는 데 도움이 됩니다.
...
2. 생성 프로세스 중에 적절한 리소스 그룹에서 시작했으므로 이미 선택되어 있어야 합니다. 다음으로, 임의로 할당된 번호를 사용하여 Purview**에 이름을 지정**합니다. 다음으로, 인스턴스에 가장 적합한 지역을 선택합니다.

   ![Purview 만들기](./images/purview-create.png)

3. **검토 및 만들기** 단추를 클릭하고 계속 진행하기 전에 유효성*** 검사를 기다립니다***.

   ![Purview 유효성 검사](./images/validation-passed.png)

4. 유효성 검사가 통과되면 만들기** 단추를 누릅니**다.
