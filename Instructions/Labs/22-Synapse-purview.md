...
> **중요**: Azure 테넌트당 사용 가능한 Microsoft Purview 계정 수를 제한하도록 Microsoft Purview가 업데이트되었습니다. 따라서 이 랩은 강사 주도 교육 과정에 사용되는 많은 호스트된 랩 환경을 포함하여 공유 테넌트 환경에서 지원되지 않습니다.

# Azure Synapse Analytics에서 Microsoft Purview 사용

Microsoft Purview를 사용하면 데이터 자산 전체에서 데이터 자산을 카탈로그화하고 포괄적인 데이터 거버넌스 솔루션의 핵심 요소인 한 데이터 원본에서 다른 데이터 원본으로 전송되는 데이터의 흐름을 추적할 수 있습니다.
...
Lake 데이터베이스는 Azure Storage의 데이터 레이크에 데이터를 저장합니다. Parquet, Delta 또는 CSV 형식 및 다양한 설정을 사용하여 스토리지를 최적화할 수 있습니다. 각 레이크 데이터베이스에는 루트 데이터 폴더를 정의하는 연결된 서비스가 있습니다.

레이크 데이터베이스는 Synapse SQL 서버리스 SQL 풀 및 Apache Spark에서 액세스할 수 있으므로 사용자는 스토리지를 컴퓨팅과 분리할 수 있습니다. 레이크 데이터베이스의 메타데이터를 사용하면 다양한 엔진이 통합 환경을 제공하고 데이터 레이크에서 지원되지 않는 추가 정보(예: 관계)를 쉽게 사용할 수 있습니다.

1. 레이크 데이터베이스를 만들려면 먼저 적절한 리소스 그룹에서 synapsexxxxxxx를 연 다음 **열기 Synapse Studio** ***열린*** 링크를 클릭합니다. 
2. 다음으로, 렌치가 있는 도구 상자를 클릭합니다. 이 도구 상자는 Synapse Analytics 작업 영역의 관리 섹션이기도 하며 전용 풀이 실행되고 있는지 확인합니다. 시작하는 데 몇 분 정도 걸릴 수 있습니다.
3. 여기에서는 배럴처럼 보이고 데이터 레이블이 있는 데이터베이스 기호를 클릭합니다.
4. 데이터 패널에서 **Data**라는 단어 오른쪽에 있는 + 기호를 클릭하고 ***Lake 데이터베이스***를 선택합니다.
   
    ![초기 lakedb 데이터베이스 만들기](./images/lakedb-configure.png)

> **참고**: **확인** 단추를 클릭하기 전에 읽고 이해해야 하는 **데이터베이스 템플릿 사용 약관에 Azure Synapse** 프롬프트가 표시됩니다.

5. 맨 오른쪽에는 속성 창 있음을 알 수 있습니다.
   1. 이름 필드에 **lakedb** 를 입력합니다.
   1. **입력 폴더**에서 폴더를 선택하고 루트/파일/데이터로 이동한 다음 **확인을** 누릅니다.

>**참고**: **입력 폴더**를 열 때 오류가 표시될 수 있습니다. 그런 경우 **확인을** 클릭하기 전에 루트 폴더를 두 번 클릭하고 데이터로 작업을 수행합니다.

   1. 이 화면 왼쪽에는 아래에 **+테이블**이 있는 **lakedb** 이름이 있는 기둥이 표시되고, 이 항목을 클릭하고 데이터 ***레이크에서***를 선택합니다.
   1. ***외부 테이블 이름***에서 **Products를 입력합니다**.
   1. ***연결된 서비스에서*** 기본 옵션을 선택합니다.
   1. ***입력 파일 또는 폴더*** 내에서 맨 오른쪽의 파일 폴더를 클릭하고 **루트 > 파일 > 데이터 >** 찾아 ***products.csv*** 선택하고 **확인을** 클릭한 다음 **계속**을 클릭합니다.
...
8. **데이터**에서 왼쪽의 **작업 영역** 영역에 있는지 확인하고 **Lake 데이터베이스** 섹션, **lakedb**를 확장한 다음 **Products** 테이블의 오른쪽***을 마우스로 마우스***로 덮고 ***상위 100개 행을*** 선택합니다.

![외부 원본에서 파생된 테이블 구조](./images/lakedb-external-table-definition.png)

> **참고**: **연결** 대상이 **기본 제공**으로 나열되어 있는지 확인하고 **master** 데이터베이스를 선택한 상태로 두거나 오른쪽의 새로 고침 단추를 클릭하고 **lakedb** 데이터베이스를 선택할 수 있습니다. 3부 명명 규칙 [데이터베이스]를 사용합니다. [스키마]. [테이블] 둘 다 작동합니다.
...
이를 설정하려면 먼저 할당된 난수에 따라 dp203-xxxxxxx라는 기본 리소스 그룹으로 돌아갑니다. **리소스 그룹** 내에 있으면 ***+ 만들기*** 단추를 클릭하여 새 서비스를 추가합니다.

1. **Microsoft Purview 서비스를** 선택한 다음 **만들기** 단추를 클릭합니다.
2. 만들기 프로세스 중에 적절한 리소스 그룹에서 시작했으므로 이미 선택되어 있어야 합니다. 다음으로, 임의로 할당된 번호를 사용하여 **Purview** 에 이름을 지정합니다. 다음으로, instance 가장 적합한 지역을 선택합니다.
...
Microsoft Purview는 관리 ID를 사용하도록 구성됩니다. 데이터 자산을 카탈로그화하려면 이 관리 ID 계정에 Azure Synapse Analytics 작업 영역 및 해당 데이터 레이크 저장소의 스토리지 계정에 액세스할 수 있어야 합니다.

1. [Azure Portal](https://portal.azure.com) 설치 스크립트에서 만든 **dp203-*xxxxxxx*** 리소스 그룹을 찾아 만든 리소스를 확인합니다. 이러한 위협은 다음과 같습니다.
    - **datalake*xxxxxxx***와 비슷한 이름의 스토리지 계정입니다.
    - *purview xxxxxxx***와 비슷한 이름의 Microsoft *Purview*계정입니다.
    - *sql xxxxxxx***와 비슷한 이름의 전용 *SQL*풀입니다.
    - *synapse xxxxxxx***와 비슷한 이름의 *Synapse*작업 영역입니다.
2. **datalake*xxxxxxx*** 스토리지 계정을 열고 **해당 Access Control(IAM)** 페이지에서 **다음과 같이 역할 할당 탭을** 봅니다.

    ![스토리지 계정에 대한 역할 할당 탭의 스크린샷](./images/storage-roles.png)

3. **+ 추가** 단추 메뉴에서 **역할 할당 추가**를 선택합니다. 
4. **역할 assigmnent 추가** 페이지의 **역할** 탭에서 "스토리지 Blob"을 검색하고 **Storage Blob 데이터 판독기** 역할을 선택합니다. 그런 다음 **, 다음** 을 선택하여 **멤버** 탭으로 이동합니다.
...
Azure Synapse Analytics 작업 영역에는 *서버리스* 및 *전용* SQL 풀 모두에 데이터베이스가 포함되며, Microsoft Purview에서 사용하는 관리 ID에는 액세스 권한이 필요합니다.

1. Azure Portal Synapse Analytics 작업 영역의 페이지에서 **개요** 탭을 봅니다. 그런 다음 **, Synapse Studio 열기** 타일에서 링크를 사용하여 새 브라우저 탭에서 Azure Synapse Studio를 엽니다. 메시지가 표시되면 로그인합니다.

    >**팁**: 또는 새 브라우저 탭에서 으로 직접 https://web.azuresynapse.net 이동하여 Azure Synapse Studio를 열 수 있습니다.
...
3. **원본 데이터 저장소** 페이지의 **연결** 목록에서 **synapse xxxxxxx-WorkspaceDefaultStorage**** 연결(작업 영역에 대한 데이터 레이크를 참조함)을 선택하고 **파일 또는 폴더**에 대해 **파일/제품/products.csv** 파일을 찾습니다. 그런 후 **다음**을 선택합니다.
...
10. Synapse Studio **모니터** 페이지를 봅니다. 그런 다음 **파이프라인 실행** 탭에서 Load_Product_Data 파이프라인의 **상태** 관찰합니다. 상태 성공으로 변경하는 데 몇 분 정도 걸릴 수 **있습니다**.
11. 파이프라인 실행이 성공적으로 완료되면 해당 이름(**Load_Product_Data**)을 선택하여 파이프라인의 활동에 대한 세부 정보를 봅니다. 파이프라인에 **Copy_* xxx**와 유사한 자동으로 파생된 이름의 **데이터 복사** 작업이 포함되어 있음을 확인합니다 *. 이 활동은 데이터 레이크의 텍스트 파일에서 **sql*xxxxxxx** 데이터베이스의 **products** 테이블로 데이터를 복사*했습니다.
...
5. **Copy_*xxx*** 자산의 **계보** 탭에서 **products.csv** 파일에서 제품 테이블로의 데이터 흐름을 보여 주는 다이어그램을 봅니**다**.
...
Azure Synapse Analytics를 Microsoft Purview와 통합하여 사용하도록 설정된 계보 추적 기능을 사용하면 데이터 저장소의 데이터가 로드된 방법과 시기 및 원본 위치를 확인할 수 있습니다.
