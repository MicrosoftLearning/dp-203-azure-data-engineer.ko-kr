...
7. 스크립트가 완료될 때까지 기다리세요. 일반적으로 약 5분이 걸리지만 경우에 따라 더 오래 걸릴 수 있습니다. 기다리는 동안 Azure Databricks 설명서의 [Databricks SQL이란?](https://docs.microsoft.com/azure/databricks/scenarios/what-is-azure-databricks-sqla) 문서를 검토합니다.
...
4. 사이드바에서 **[D] 데이터 과학 및 엔지니어링**을 확장하고 **[S] SQL**을 선택합니다. SQL 가상 사용자를 반영하도록 포털 인터페이스를 변경합니다(이 가상 사용자는 *프리미엄 계층* 작업 영역에서만 사용할 수 있음).
5. **시작** 창에서 **SQL Warehouse 검토**를 선택합니다(또는 사이드바에서 **SQL Warehouse** 선택).
6. 작업 영역에 **Starter Warehouse**라는 SQL Warehouse가 이미 포함되어 있는지 확인합니다.
7. SQL Warehouse의 **작업**( **&#8285;** ) 메뉴에서 **편집**을 선택합니다. 그런 다음, **클러스터 크기** 속성을 **2X-Small**로 설정하고 변경 내용을 저장합니다.
8. **시작** 단추를 사용하여 SQL Warehouse를 시작합니다(1~2분 정도 걸릴 수 있음).
...
1. SQL Warehouse가 *실행* 중이면 왼쪽의 **SQL 웨어하우스 검토** 창을 닫습니다. 그런 다음, 사이드바에서 **SQL 편집기**를 선택합니다.
2. **스키마 브라우저** 창에서 하이브 메타스토어에 **기본**이라는 데이터베이스가 포함되어 있는지 확인합니다.
3. **새 쿼리** 창에 다음 SQL 코드를 입력합니다.

    ```sql
    CREATE SCHEMA adventureworks;
    ```
4. **모두 실행** 단추를 사용하여 SQL 코드를 실행합니다.
5. 코드가 성공적으로 실행되면 **스키마 브라우저** 창에서 **&#8635;** 단추를 사용하여 목록을 새로 고칩니다. 그런 다음, **기본** 데이터베이스를 선택하여 데이터베이스 목록을 표시하고 **adventureworks**를 선택합니다. 데이터베이스가 만들어졌지만 테이블이 없습니다.

테이블에 **기본** 데이터베이스를 사용할 수 있지만 분석 데이터 저장소를 빌드할 때는 특정 데이터에 대한 사용자 지정 데이터베이스를 만드는 것이 가장 좋습니다.

## 테이블 만들기

1. 사이드바에서 **(+) 새로 만들기**를 선택한 다음, **파일 업로드**를 선택합니다.
2. **데이터 업로드** 영역에서 **찾아보기**를 선택합니다. 그런 다음, **열기** 대화 상자에서 `https://raw.githubusercontent.com/MicrosoftLearning/dp-203-azure-data-engineer/master/Allfiles/labs/26/data/products.csv`를 입력하고 **열기**를 선택합니다.

    > **팁**: 브라우저 또는 운영 체제에서 **파일** 상자에 URL 입력을 지원하지 않는 경우 CSV 파일을 컴퓨터에 다운로드한 다음, 저장한 로컬 폴더에서 업로드합니다.

3. **데이터 업로드** 페이지에서 **adventureworks** 데이터베이스를 선택하고 테이블 이름을 **제품**으로 설정합니다. 그런 다음, **테이블 만들기**를 선택합니다.
4. 테이블이 만들어지면 세부 정보를 검토합니다.
...
2. **스키마 브라우저** 창에서 **adventureworks** 데이터베이스가 선택되어 있고 **제품** 테이블이 나열되어 있는지 확인합니다.
3. **새 쿼리** 창에 다음 SQL 코드를 입력합니다.
