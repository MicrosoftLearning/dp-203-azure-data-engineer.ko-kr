...
> **팁**: *프리미엄* 또는 *Trial* Azure Databricks 작업 영역이 이미 있는 경우 이 절차를 건너뛸 수 있습니다.
...
5. 코드가 성공적으로 실행되면 **스키마 브라우저** 창에서 바닥에 있는 새로 고침 단추를 사용하여 목록을 새로 고칩니다. 그런 다음 **hive_metastore** 및 **adventureworks**를 확장하고 데이터베이스가 만들어졌지만 테이블이 없는지 확인합니다.
...
1. Azure Databricks 작업 영역 포털의 사이드바에서 ** 새로 만들기**를 선택한 다음 **파일 업로드**를 선택하고 컴퓨터에 다운로드한 **products.csv** 파일을 업로드합니다.
...
2. **스키마 브라우저** 창에서 **hive_metastore** and **adventureworks**를 확장하고 **products** 테이블이 나열되었는지 확인합니다.
3. **새 쿼리** 창에 다음 SQL 코드를 입력합니다.
...
4. **시각화 위젯 추가** 대화 상자에서 **제품 및 범주** 쿼리를 선택합니다. 그런 다음, **새 시각화 만들기**를 선택하고 제목을 **범주별 제품**으로 설정한 다음 **시각화 만들기**를 선택합니다.
