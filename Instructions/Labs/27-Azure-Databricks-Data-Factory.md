...
1. 페이지 위쪽의 사용자 이름 옆에 있는 **&#8942;** 메뉴에서 **가져오기**를 선택합니다. 그런 다음 **가져오기** 대화 상자에서 **URL** 을 선택하고 에서 Notebook을 가져옵니다. `https://github.com/MicrosoftLearning/dp-203-azure-data-engineer/raw/master/Allfiles/labs/27/Process-Data.ipynb`
...
1. Azure Databricks 포털의 오른쪽 위 메뉴 모음에서 사용자 이름을 선택한 다음, 드롭다운에서 **사용자 설정을** 선택합니다.
1. **사용자 설정** 페이지에서 **개발자**를 선택합니다. 그런 다음 **액세스 토큰 옆에 있는** **관리를** 선택합니다.
1. **새 토큰 생성을** 선택하고 Data *Factory* 주석과 빈 수명을 사용하여 새 토큰을 생성합니다(토큰이 만료되지 않도록). *Done***을 선택 <u>하기 전에</u> 표시될 때 **토큰을 복사해야 합니다.
