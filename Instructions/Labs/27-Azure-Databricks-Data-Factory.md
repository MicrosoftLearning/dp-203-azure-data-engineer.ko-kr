...
1. Azure Databricks 포털의 오른쪽 위 메뉴 모음에서 사용자 이름을 선택한 다음 드롭다운에서 사용자 설정** 선택합니다**.
1. **사용자 설정** 페이지에서 개발자**를 선택합니다**. 그런 다음 액세스 토큰 옆에 **있는** 관리를** 선택합니다**.
1. 새 토큰** 생성을 선택하고 **주석 *Data Factory* 및 빈 수명을 사용하여 새 토큰을 생성합니다(토큰이 만료되지 않도록). *Done***을(를) 선택하기 전에<u> 표시될 </u>때 **토큰을 복사합니다.
