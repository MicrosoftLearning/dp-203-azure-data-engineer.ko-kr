...
5. 이 경우 데이터 파일에는 첫 번째 행에 열 이름이 포함됩니다. 따라서 여기에 표시된 대로 쿼리를 수정하여 `WITH` 절에 `HEADER_ROW = TRUE` 매개 변수를 추가합니다(이전 매개 변수 다음에 쉼표 추가를 잊지 마세요).
...
## CETAS(CREATE EXTERAL TABLE AS SELECT) 문을 사용하여 데이터 변환
